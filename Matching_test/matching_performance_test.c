...
// -I../3rd-party/openpmix/include matching_performance_test.c -Wno-format
...
int main(int argc, char **argv)
{
    ...
    for (long i = 0; i < num_ops; ++i) {
        ...
        if (rand() % 2 == 1) {
            ...
            umq_dequeues++;
            if (msg_found) {
                printf("Matched in UMQ (%d,%d)\n",tag,src);
                // matched => do nothing else
                uq_size--;
                custom_match_umq_remove_hold(uq, hold_prev, hold_elem,
                                             hold_index); // actually remove
                assert(get_match_or_insert(matching_map, tag, src, payload, true));
            } else {
                printf("add to PRQ (%d,%d)\n",tag,src);
                ...
                if (pq_size > pq_max)
                    pq_max = pq_size;
                assert(NULL == get_match_or_insert(matching_map, tag, src, payload, true));
            }
        }
    }
